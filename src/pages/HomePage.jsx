...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-hidden" style={{ backgroundColor: '#FFF9F0' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        {FLOATING_ICONS.map((item, i) => (
          <motion.div
            ...
            animate={{ y: [0, -(14 + i * 2), 0], rotate: [0, i % 2 === 0 ? 8 : -8, 0] }}
      ...
      >
        ...
        >
          ...
          <span style={{ fontFamily: 'var(--font-dm)', fontSize: '13px', color: '#FF6B6B', fontWeight: 600 }}>
            AI-Powered Birthday Wishes
          </span>
        </motion.div>

        <h1
          className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.2rem] font-bold mb-4 leading-tight"
          style={{ fontFamily: 'var(--font-playfair)', color: '#2D2D2D' }}
        >
          Tengeneza Birthday Wish{' '}
          <span
            style={{
              background: 'linear-gradient(135deg, #FF6B6B 0%, #FF9A5C 40%, #FFE66D 100%)',
        ...
        <p
          className="text-base md:text-lg max-w-sm mx-auto leading-relaxed"
          style={{ fontFamily: 'var(--font-dm)', color: '#6B6B6B' }}
        >
          Mfurahishe mtu unayempenda kwa njia ya kipekee na AI ✨
        </p>
      ...
      <div className="px-4 sm:px-6 max-w-[580px] mx-auto relative z-10">
          ...
          >
            ...
            <div
              style={{
                ...
                right: 0,
                height: '4px',
                background: 'linear-gradient(90deg, #FF6B6B, #FFE66D, #A8EDEA, #FF6B6B)',
                backgroundSize: '200% auto',
                animation: 'shimmer 3s linear infinite',
              }}
            />
        ...
        >
          {[
            { step: '1', label: 'Jaza Fomu', icon: '✏️' },
            { step: '2', label: 'AI Inaandika', icon: '🤖' },
            { step: '3', label: 'Share Link', icon: '🔗' },
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              <div
                className="flex items-center gap-1.5"
                style={{ fontFamily: 'var(--font-dm)', fontSize: '12px', color: '#6B6B6B' }}
              >
                <span
                  className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                  style={{ background: 'linear-gradient(135deg,#FF6B6B,#FFE66D)', color: '#fff', boxShadow: '0 2px 8px rgba(255,107,107,0.3)' }}
                >
                  {item.step}
                </span>
              ...
              {i < 2 && (
                <div style={{ width: '20px', height: '1px', backgroundColor: '#E8D5C4' }} />
              )}
            </div>
          ))}
        </motion.div>
